...
async def evaluate_command_handler(c: Client, m: Message):
    ...
    if "-s" in user_args and TGubot.log_chat:
        if "-paste" in user_args:
            url = await Paste(final_output).paste()
            msg = await c.send_message(
                TGubot.log_chat, f"{cmd_} \n<b>Pasted to :</b> {url}"
            )
            ...
            msg = await c.send_document(TGubot.log_chat, file_path, caption=cmd)
            if os.path.exists(file_path):
                os.remove(file_path)
        else:
            msg = await c.send_message(TGubot.log_chat, final_output)
...
async def terminal(c: Client, m: Message):
    ...
    _out_text = out_text
    _out_text += f'\n\n<b>{TGubot.get_string("OUTPUT")}</b>\n<code>{output or TGubot.get_string("NO_OUTPUT")}</code>\n'
    ...
    if "-s" in user_args and TGubot.log_chat:
        if "-paste" in user_args:
            ...
            msg = await c.send_message(
                TGubot.log_chat, f"{caption_} \n<b>Pasted to :</b> {url}"
            )
        elif "-file" in user_args or len(out_text) >= TGLIMITS.MESSAGE_TEXT:
            file_path = f"out_bash_{m.id}.txt"
            async with aiofiles.open(file_path, "w") as f:
                await f.write(_out_text)
            msg = await c.send_document(TGubot.log_chat, file_path, caption=caption_)
            if os.path.exists(file_path):
                os.remove(file_path)
        else:
            msg = await c.send_message(TGubot.log_chat, _out_text)
        return await m.edit_msg(f"<b>OUTPUT :</b> <a href='{msg.link}'>VIEW</a>")
...
async def logs(c: Client, m: Message):
    ...
    if "-s" in log__args_ and TGubot.log_chat:
        msg = (
            await c.send_document(
                TGubot.log_chat,
                log_file_,
                file_name="TGubot_logs.txt",
                caption="LOGS of your TGubot Userbot.",
            )
            if "_paste" not in log__args_
            else await c.send_message(TGubot.log_chat, (await paste_logs(log_file_)))
        )
