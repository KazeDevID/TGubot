...
@TGubot.register_on_cmd(
    "dpfs",
    cmd_help={"help": "Cmd yang dinonaktifkan untuk pengguna Sudo!", "example": "dpfs eval"},
    requires_input=True,
)
async def disabled_ps_func(c: Client, m: Message):
    ...
    await TGubot.config.sync_env_to_db("DISABLED_SUDO_CMD_LIST", input_, push_=True)
    await msg.edit_msg("DISABLED_SUDO_CMD", string_args=(input_))


@TGubot.register_on_cmd(
    "rmdfs",
    cmd_help={"help": "Cmd yang dinonaktifkan dari pengguna Sudo","example": "dpfs eval"},
    requires_input=True,
)
async def remove_disabled_ps_func(c: Client, m: Message):
    ...
    if "," in input_:
        input_ = input_.split(",")
    await TGubot.config.unsync_env_to_db("DISABLED_SUDO_CMD_LIST", input_)
...
@TGubot.register_on_cmd(
    "addsudo",
    cmd_help={
        "help": "Tambahkan pengguna ke daftar Sudo, perlu restart setelah selesai!",
...
async def add_sudo_func(c: Client, m: Message):
    ...
    try:
        ...
        return await msg.edit_msg("INVALID_USER")
    if user_id.id in (await TGubot.config.get_sudo()):
        return await msg.edit_msg("ALREADY_IN_SUDO")
    await TGubot.config.add_sudo(user_id.id)
...
@TGubot.register_on_cmd(
    "rmsudo",
    cmd_help={
        "help": "hapus sudo dari daftar sudo, perlu dimulai ulang untuk mencerminkan perubahan",
...
async def rm_sudo_func(c: Client, m: Message):
    msg = await m.handle_message("PROCESSING")
    user, _, is_channel = m.get_user
    acg = await TGubot.config.get_sudo()
    if user_args := m.user_args:
        if "-a" in user_args:
            ...
            for i in acg:
                try:
                    await TGubot.config.del_sudo(i)
                    count += 1
    ...
    await TGubot.config.del_sudo(user_id.id)
...
async def add_sudo_func(c: Client, m: Message):
    msg = await m.handle_message("PROCESSING")
    users_ = await TGubot.config.get_sudo()
    if not users_:
