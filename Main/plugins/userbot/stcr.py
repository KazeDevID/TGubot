...
async def getstickerfromcollection(name_: str):
    try:
        return await TGubot.bot.invoke(
...
async def kang_fun(c: Client, m: Union[Message, Rmessage]):
    ...
    try:
        if m.from_user.id in cached_peer:
            ...
            peer_ = await TGubot.bot.resolve_peer(m.from_user.id)
            cached_peer[m.from_user.id] = peer_
    except (KeyError, PeerIdInvalid):
        return await msg.edit_msg(
            "START_YOUR_ASSISTANT", string_args=(TGubot.bot_info.username)
        )
    ...
    if m.reply_to_message.photo or (
        ...
        file_ = await TGubot.bot.upload_doc(new_photo, peer_)
        ...
        sticker_limit = 50
        file_ = await TGubot.bot.upload_doc(down_, peer_, force_file=True)
    ...
    while True:
        pack_ += 1
        pack_m = f"TGubot_{m.from_user.id}" + (
            f"_{sticker_name.lower()}" if sticker_name != "" else ""
        )
        pack_m += f"_{pack_}_by_{TGubot.bot_info.username}"
        title = f"{my_name}'s " + (
        ...
        if not stick_:
            stick_ = await TGubot.bot.invoke(
...
async def get_pack_(c: Client, m: Message):
    ...
    while True:
        ...
        pack_m = f"TGubot_{str(m.from_user.id)}" + (
