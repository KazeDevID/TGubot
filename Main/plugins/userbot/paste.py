...
async def paste(c: Client, m: Message):
    ...
    if "-m" in m.user_args:
        _hash = random_hash()
        TGubot.local_db.add_to_col("paste", {_hash: text})
        try:
            results = await c.get_inline_bot_results(
                TGubot.bot_info.username, f"paste_menu:{_hash}"
            )
            ...
            return await m.edit_msg(TGubot.get_string("INLINE_NOT_ALLOWED"))
    elif val := (set(m.user_args) & set(Paste().all_bins)):
        name, link = await Paste(text, service=val[0]).paste()
        try:
            results = await c.get_inline_bot_results(
                TGubot.bot_info.username, f"paste_url:{link}"
            )
            await c.send_inline_bot_result(
                chat_id=m.chat.id,
                query_id=results.query_id,
            ...
            await m.edit_msg(TGubot.get_string("PASTE_TEXT").format(name, link))
    else:
        name, link = await Paste(text).paste()
        await m.edit(
            TGubot.get_string("PASTE_TEXT").format(link, name),
