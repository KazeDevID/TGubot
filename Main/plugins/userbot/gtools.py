...
gban_db = TGubot.db.make_collection("Gbans")
USER_CHAT_CACHE = []
...
@TGubot.on_message((filters.group | filters.channel) & ~filters.private, group=2)
@is_gbanned
async def gwatch_(c: Client, m: Message):
    ...
    if await TGubot.config.get_env("ENABLE_GLOGS") and TGubot.log_chat:
        chat_ = m.chat.username or m.chat.id
        gbanned_ = await TGubot.get_string(
            "GBANNED_USER_JOINED", args=(m.from_user.mention, chat_, m.id)
        )
        await c.send_message(TGubot.log_chat, gbanned_)
