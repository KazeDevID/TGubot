...
welcome_db = TGubot.db.make_collection("WELCOME")


@TGubot.register_on_cmd(
    ["savewelcome", "addwelcome"],
    cmd_help={
        "help": "Simpan pesan selamat datang dalam obrolan.",
        "example": "savewelcome Hello {first} , apa kabar?",
    },
    requires_reply=True,
)
async def add_welcome_cmd(c: Client, m: Message):
    ...
    save_welcome = await reply_msg.copy(int(TGubot.log_chat))
...
async def welcome_settings_cmd(c: Client, m: Message):
    ...
    if not welcome_data or not TGubot.log_chat:
        return await m.handle_message("NO_WELCOME_SAVED")
    await m.handle_message("CURRENT_WELCOME")
    await c.copy_message(
        from_chat_id=int(TGubot.log_chat),
...
async def welcome_users(c: Client, m: Message):
    ...
    welcome_message = await c.get_messages(TGubot.log_chat, welcome_data["msg_id"])
