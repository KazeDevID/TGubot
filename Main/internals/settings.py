...
@TGubot.bot.on_callback_query(filters.regex("configs_home"))
@log_errors
...
@TGubot.bot.on_callback_query(filters.regex("settings_menu"))
@log_errors
async def settings_menu_cb_handler(c: Client, m: Message):
    await m.reply(
        TGubot.get_string("SETTINGS_TEXT") or "<b>Settings</b>",
...
def get_sessions_buttons(page=1) -> Tuple[List[InlineKeyboardButton], bool, int]:
    ...
    buttons = [
        ...
        for index, i in enumerate(TGubot.clients)
...
@TGubot.bot.on_message(
    filters.command("settings", "/") & filters.user(TGubot.auth_users)
)
@log_errors
...
@TGubot.bot.on_callback_query(filters.regex("sessions_list_(\\d+)$"))
@log_errors
...
@TGubot.bot.on_callback_query(filters.regex("refresh_session_info_(\\d+)$"))
@log_errors
async def refresh_session_info_cb_handler(c: Client, cb: CallbackQuery):
    index = int(cb.matches[0].group(1))
    TGubot.ourselves[index] = await TGubot.clients[index].get_me()
    await cb.answer("Data refreshed!")
    await sessions_info_cb_handler(c, cb)


@TGubot.bot.on_callback_query(filters.regex("unlink_session_(\\d+)$"))
@log_errors
async def unlink_session_cb_handler(c: Client, cb: CallbackQuery):
    ...
    await TGubot.bot.delete_messages(temp.chat.id, (temp.id, confirmation.id))
    ...
    await temp.delete()
    await TGubot.remove_session(int(cb.matches[0].group(1)))
