# Copyright (C) 2022-X-SyntaxError, < https://github.com/X-SyntaxError
...
class CustomClientMethods:
    ...
    async def invoke(self, *args, **kwargs):
        ...
        while True:
            try:
                ...
                if max_count > mmax_:
                    raise e
                TGubot.log(f"[{e.__class__.__name__}]: sleeping for - {e.x + 3}s.")
    ...
    async def check_my_perm(self, msg, perm_type):
        ...
        if (
            ...
            and TGubot.SELF_PERMISSION_CACHE.get(chat.id)
            and TGubot.SELF_PERMISSION_CACHE.get(chat.id).get(my_id)
        ):
            perms_json = TGubot.SELF_PERMISSION_CACHE[chat.id][my_id]
        else:
            try:
            ...
            if not TGubot.SELF_PERMISSION_CACHE.get(chat.id):
                TGubot.SELF_PERMISSION_CACHE[chat.id] = {}
            TGubot.SELF_PERMISSION_CACHE[chat.id][my_id] = perms_json
