...
class Message:
    ...
    async def handle_message(self, text_, **kwargs):
        sudo_users = TGubot.config.SUDO_USERS
        if self._client.myself.id == TGubot.bot_info.id:
    ...
    async def delete_if_sudo(self, **kwargs):
        sudo_ = TGubot.config.SUDO_USERS
