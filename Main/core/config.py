...
class BaseConfig(object):
    ...
    DEFAULT_REPO = "https://github.com/X-SyntaxError/TGubot"
    HEROKU_API_KEY = getenv("HEROKU_API_KEY")
    REPO = getenv("CUSTOM_REPO") or DEFAULT_REPO
    DEFAULT_PM_IMAGE = "./Main/assets/images/pmpermit.jpg"
    DEFAULT_PM_TEXT = """<b>TGubot's PM BOT</b>
<i>Hello, {mention}. Welcome to {mymention}'s PM. Harap Nyatakan kekhawatiran Anda dan tunggu dia merespons.</i>

<i>You currently <code>{warns}/{max_warns}</code> Peringatan. Jika Anda akhirnya melebihi batas peringatan, Anda akan langsung diblokir sampai pemberitahuan lebih lanjut!</i>
"""
    APPROVED_DICT: dict = {}
    CUSTOM_PM_TEXT: dict = {}
    UPDATE_ON_STARTUP = (
    ...
    def pop_session(self, index: int) -> Optional[str]:
        if len(self.SESSIONS) == 0:
            raise Exception("Tidak ada sesi untuk muncul")
        if index <= len(self.SESSIONS):
            for trials in range(5):
                try:
                    popped = self.SESSIONS.pop(index)
                    logging.info("Sesi Berhasil Muncul.")
                    break
                except IndexError as e:
                    logging.warning(f"Index {index} berada di luar jangkauan. Mencoba lagi...")
                    if trials == 4:
                        raise Exception("Tidak dapat membuka session")from e
                    continue
        else:
            print("\n")
            logging.error("Session index diluar jangkauan.")
            quit(1)
        ...
        with open(env_path, "w") as f:
            ...
            f.write(new_data)
        logging.info("Sesi yang tidak stabil berhasil dihapus")
        return popped
    ...
    def append_session(self, session: str) -> None:
        if session not in self.SESSIONS:
            ...
            with open(env_path, "w") as f:
                ...
                f.write(new_data)
            logging.info("Sesi baru berhasil ditulis ke .env")
            return
...
class Config(BaseConfig):
    def __init__(
        ...
        if env_col is None:
            raise NoDatabaseConnected(
                "Tolong tambahkan database uri, Agar Userbot dapat berfungsi!"
            )
    ...
    async def add_env_to_db(
        ...
        if self.DEBUG:
            logging.info("DEBUG mode diaktifkan, melewatkan sinkronisasi db...")
